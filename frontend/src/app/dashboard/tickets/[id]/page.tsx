...
export default function DashboardViewTicketPage() {
  ...
  useEffect(() => {
    ...
    const doUseEffect = async (accessToken: string, ticketId: string) => {
      try {
        setIsQrCodeLoading(true);
        setError(undefined);

        setTicket(await getTicket(accessToken, ticketId));
        setQrCodeUrl(
          URL.createObjectURL(await getTicketQr(accessToken, ticketId)),
        );
      } catch (err) {
