...
export default function ManageEventForm({ eventId }: ManageEventFormProps) {
  ...
  useEffect(() => {
    if (isEditMode && !isLoading && user?.access_token && eventId) {
      const fetchEvent = async () => {
        const event: EventDetails = await getEvent(user.access_token, eventId);
        setEventData({
          id: event.id,
          name: event.name,
